...
uint8 LCD1_initVar = 0u;
    ...
    volatile uint8 LCD1_txBuffer[LCD1_TX_BUFFER_SIZE];
    ...
    uint8 LCD1_errorStatus = 0u;
    volatile uint8 LCD1_rxBuffer[LCD1_RX_BUFFER_SIZE];
    ...
    volatile uint8 LCD1_rxBufferOverflow   = 0u;
        ...
        volatile uint8 LCD1_rxAddressMode = LCD1_RX_ADDRESS_MODE;
...
*  LCD1_Start() sets the initVar variable, calls the
*  LCD1_Init() function, and then calls the
*  LCD1_Enable() function.
*
* Parameters:
*  None.
*
...
*  and set to one (1u) the first time LCD1_Start() is called. This
*  allows for component initialization without re-initialization in all
*  subsequent calls to the LCD1_Start() routine.
*
* Reentrant:
*  No.
*
...
*  dialog settings. It is not necessary to call LCD1_Init() because
*  the LCD1_Start() API calls this function and is the preferred
...
void LCD1_Init(void) 
{
        ...
        #if (LCD1_RXHW_ADDRESS_ENABLED)
            LCD1_SetRxAddressMode(LCD1_RX_ADDRESS_MODE);
            LCD1_SetRxAddress1(LCD1_RX_HW_ADDRESS1);
            LCD1_SetRxAddress2(LCD1_RX_HW_ADDRESS2);
...
*  to call LCD1_Enable() because the LCD1_Start() API
...
void LCD1_Enable(void) 
{
    ...
    enableInterrupts = CyEnterCriticalSection();
        ...
        #if (LCD1_TX_INTERRUPT_ENABLED)
            LCD1_ClearPendingTxInt(); /* Clear history of TX_NOT_EMPTY */
            LCD1_EnableTxInt();
        #endif /* (LCD1_TX_INTERRUPT_ENABLED) */
     #endif /* (LCD1_TX_INTERRUPT_ENABLED) */

    #if (LCD1_INTERNAL_CLOCK_USED)
        LCD1_IntClock_Start();  /* Enable the clock */
    #endif /* (LCD1_INTERNAL_CLOCK_USED) */
...
void LCD1_Stop(void) 
{
    ...
    #if (LCD1_INTERNAL_CLOCK_USED)
        LCD1_IntClock_Stop();   /* Disable the clock */
    #endif /* (LCD1_INTERNAL_CLOCK_USED) */
...
uint8 LCD1_ReadControlRegister(void) 
{
    ...
    #endif /* (LCD1_CONTROL_REG_REMOVED) */
}
...
void  LCD1_WriteControlRegister(uint8 control) 
{
    ...
    *      LCD1_RX_STS_FIFO_NOTEMPTY    Interrupt on byte received.
    *      LCD1_RX_STS_PAR_ERROR        Interrupt on parity error.
    *      LCD1_RX_STS_STOP_ERROR       Interrupt on stop error.
    *      LCD1_RX_STS_BREAK            Interrupt on break.
    *      LCD1_RX_STS_OVERRUN          Interrupt on overrun error.
    *      LCD1_RX_STS_ADDR_MATCH       Interrupt on address match.
    *      LCD1_RX_STS_MRKSPC           Interrupt on address detect.
    *
    * Return:
    *  None.
    *
    ...
    *  LCD1_rxBufferLoopDetect - cleared if loop condition was detected
    *     in RX ISR.
    *
    * Reentrant:
    *  No.
    ...
    *******************************************************************************/
    uint8 LCD1_ReadRxData(void) 
    {
        ...
        locRxBufferRead  = LCD1_rxBufferRead;
        locRxBufferWrite = LCD1_rxBufferWrite;

        if( (LCD1_rxBufferLoopDetect != 0u) || (locRxBufferRead != locRxBufferWrite) )
        {
            rxData = LCD1_rxBuffer[locRxBufferRead];
            locRxBufferRead++;

            if(locRxBufferRead >= LCD1_RX_BUFFER_SIZE)
            {
                locRxBufferRead = 0u;
            }
            /* Update the real pointer */
            LCD1_rxBufferRead = locRxBufferRead;
            ...
            if(LCD1_rxBufferLoopDetect != 0u)
            {
                ...
                #endif /* ((LCD1_RX_INTERRUPT_ENABLED) && (LCD1_FLOW_CONTROL != 0u)) */
            }
        }
        else
    ...
    *  LCD1_RX_STS_FIFO_NOTEMPTY.
    *  LCD1_RX_STS_FIFO_NOTEMPTY clears immediately after RX data
    *  register read.
    *
    * Global Variables:
    ...
    *
    * Summary:
    *  Returns the last received byte of data. LCD1_GetChar() is
    ...
    *******************************************************************************/
    uint16 LCD1_GetByte(void) 
    {
        ...
        /* Protect variables that could change on interrupt */
        LCD1_DisableRxInt();
        locErrorStatus = (uint16)LCD1_errorStatus;
        LCD1_errorStatus = 0u;
        LCD1_EnableRxInt();
        return ( (uint16)(locErrorStatus << 8u) | LCD1_ReadRxData() );
    #else
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
        ...
        {
            size = (LCD1_RX_BUFFER_SIZE - LCD1_rxBufferRead) + LCD1_rxBufferWrite;
        }
    ...
    *   LCD1_TX_STS_COMPLETE        Interrupt on TX byte complete
    *   LCD1_TX_STS_FIFO_EMPTY      Interrupt when TX FIFO is empty
    *   LCD1_TX_STS_FIFO_FULL       Interrupt when TX FIFO is full
    *   LCD1_TX_STS_FIFO_NOT_FULL   Interrupt when TX FIFO is not full
    *
    * Return:
    *  None.
    *
    ...
    *******************************************************************************/
    void LCD1_WriteTxData(uint8 txDataByte) 
    {
        ...
        if(LCD1_initVar != 0u)
        {
            ...
            {
                if(LCD1_txBufferWrite >= LCD1_TX_BUFFER_SIZE)
                {
    ...
    *******************************************************************************/
    void LCD1_PutChar(uint8 txDataByte) 
    {
        ...
        */
        ...
        #if ((LCD1_TX_BUFFER_SIZE > LCD1_MAX_BYTE_VALUE) && (CY_PSOC3))
            /* Disable TX interrupt to protect variables from modification */
            LCD1_DisableTxInt();
        #endif /* (LCD1_TX_BUFFER_SIZE > LCD1_MAX_BYTE_VALUE) && (CY_PSOC3) */

            locTxBufferWrite = LCD1_txBufferWrite;
            locTxBufferRead  = LCD1_txBufferRead;
        ...
        {
            if(locTxBufferWrite >= LCD1_TX_BUFFER_SIZE)
            {
                locTxBufferWrite = 0u;
            }
            /* Add to the software buffer. */
            LCD1_txBuffer[locTxBufferWrite] = txDataByte;
            locTxBufferWrite++;
            ...
            LCD1_txBufferWrite = locTxBufferWrite;

        #if ((LCD1_TX_BUFFER_SIZE > LCD1_MAX_BYTE_VALUE) && (CY_PSOC3))
            LCD1_EnableTxInt();
        #endif /* (LCD1_TX_BUFFER_SIZE > LCD1_MAX_BYTE_VALUE) && (CY_PSOC3) */

            if(0u != (LCD1_TXSTATUS_REG & LCD1_TX_STS_FIFO_EMPTY))
            {
                /* Trigger TX interrupt to send software buffer */
                LCD1_SetPendingTxInt();
            }
        }
    ...
    *******************************************************************************/
    void LCD1_PutString(const char8 string[]) 
    {
        ...
        if(LCD1_initVar != 0u)
        {
            ...
            {
                LCD1_PutChar((uint8)string[bufIndex]);
                bufIndex++;
            }
        }
    }
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
        ...
        {
            size = (LCD1_TX_BUFFER_SIZE - LCD1_txBufferRead) +
                    LCD1_txBufferWrite;
        }
    ...
    *   LCD1_SEND_BREAK - Initialize registers for break, send the Break
    *       signal and return immediately.
    *   LCD1_WAIT_FOR_COMPLETE_REINIT - Wait until break transmission is
    *       complete, reinitialize registers to normal transmission mode then return
    *   LCD1_REINIT - Reinitialize registers to normal transmission mode
    *       then return.
    *   LCD1_SEND_WAIT_REINIT - Performs both options: 
    *      LCD1_SEND_BREAK and LCD1_WAIT_FOR_COMPLETE_REINIT.
    ...
    *     SendBreak(0);     - initialize Break signal transmission
    *         Add your code here to use CPU time
    *     When interrupt appear with LCD1_TX_STS_COMPLETE status:
    *     SendBreak(2);     - complete Break operation
    *
    * Side Effects:
    *  The LCD1_SendBreak() function initializes registers to send a
    *  break signal.
    ...
    void LCD1_SendBreak(uint8 retMode) 
    {
        ...
        if(LCD1_initVar != 0u)
        {
                ...
                (retMode == LCD1_SEND_WAIT_REINIT) )
            {
                ...
                #if(LCD1_TXCLKGEN_DP)
                    txPeriod = LCD1_TXBITCLKTX_COMPLETE_REG;
                    LCD1_TXBITCLKTX_COMPLETE_REG = LCD1_TXBITCTR_BREAKBITS;
                #else
                    txPeriod = LCD1_TXBITCTR_PERIOD_REG;
            ...
            #if(LCD1_TXCLKGEN_DP)
                LCD1_TXBITCLKTX_COMPLETE_REG = txPeriod;
            #else
                LCD1_TXBITCTR_PERIOD_REG = txPeriod;
            #endif /* End LCD1_TXCLKGEN_DP */
    ...
    * Parameters:
    *  addressMode: 
    *       LCD1_SET_SPACE - Configure the transmitter to send the next
    *                                    byte as a data.
    *       LCD1_SET_MARK  - Configure the transmitter to send the next
    *                                    byte as an address.
    *
    * Return:
    *  None.
    *
    * Side Effects:
    *  This function sets and clears LCD1_CTRL_MARK bit in the Control
    *  register.
    *
    *******************************************************************************/
